...
def expected_sets_won(my_rating, opponent_rating, n_sets):
    ...
    win_sets_probabilities = np.array([binom_koeff(i, n_sets) * (win_probability ** i) * ((1 - win_probability) ** (n_sets - i)) for i in range(n_sets + 1)])
    sets_counts = np.arange(n_sets + 1)
    return np.sum(win_sets_probabilities * sets_counts)


def count_new_rating(old_rating, expected_sets_won, real_sets_won, ball_koeff):
    norm_koeff = 10 if old_rating >= 2400 else 20
    return old_rating + norm_koeff * ball_koeff * (real_sets_won - expected_sets_won)
...
def apply_elo_law(winner, looser, 
    ...
    winner_new_rating = count_new_rating(winner_rating, won_expected, winner_sets, ball_koeff)
    looser_new_rating = count_new_rating(looser_rating, lost_expected, looser_sets, ball_koeff)
...
def update_ratings_with_game(ratings, game):
    ...
    ball_koeffs = {'blue':0.5, 'red':0.7, 'yellow':1.0}
    ball_koeff = ball_koeffs[game.Ball]
...
def count_ratings(date):
    ...
    data = {'Rating':[1400 for i in range(len(all_players))]}
